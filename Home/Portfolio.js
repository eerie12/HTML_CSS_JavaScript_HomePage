...
function scrollFunction(){
    ...
    if(document.documentElement.scrollTop > 70){
        if(!header.classList.contains('navbar-fixed')){
            ...
            document.getElementsByTagName('bodt')[0].style.marginTop = '70px';
...
function plusImageSlides(n){
    clearInterval(imageTimer);//intervalをクリアした後再びinterval->使用者の命令が無視されない。
...
document.getElementById('firstDot').addEventListener('click', currentImageSlides.bind(null,1));
document.getElementById('secondDot').addEventListener('click', currentImageSlides.bind(null,2));
document.getElementById('thirdDot').addEventListener('click', currentImageSlides.bind(null,3));
document.getElementById('fourthDot').addEventListener('click', currentImageSlides.bind(null,4));
